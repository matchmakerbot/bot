...
class newTeamGuid {
  constructor(guildId) {
    (this.id = guildId), (this.teams = []);
  }
}

class newGuild {
  constructor(guildId) {
    (this.id = guildId), (this.game = ""), (this.whitelists = []);
    this.channels = {};
  }
}
...
MongoDB.connectdb((err) => {
      ...
      if (channelMode[message.guild.id] === undefined) {
        return await guildsCollection
          ...
          .then((dataDB) => {
            ...
            channelMode[message.guild.id] =
              dataDB[0].channels[message.channel.id];
          });
      }
  ...
  client.on("guildCreate", async (guild) => {
    ...
    const teamsInfo = new newTeamGuid(guild.id);
