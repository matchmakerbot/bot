...
const execute = async (message) => {
  ...
  const findGuildTeams = await teamsCollection
    .find({
      id: message.guild.id,
    })
  ...
  const membersJoinInDatabase = `teams.${findGuildTeams.indexOf(
    findGuildTeams.find((e) => e.name === messageArgs(message))
  )}.members`;
  ...
  const givewinLose = async (score, pos) => {
    for (const games of ongoingGames) {
      for (const team of findGuildTeams) {
        if (
          games[pos].name === team.name &&
          games.map((e) => e.name).includes(teamsInfo().name) &&
          games[2].guild === message.guild.id
        ) {
          const channelPos = findGuildTeams[findGuildTeams.indexOf(team)].channels
            .map((e) => e.channelID)
            .indexOf(channel_ID);
          ...
          await teamsCollection.update(
            ...
            },
            {
              $set: {
                ...
                [lose]:
                  status === "losses" ? team.channels[channelPos].losses + 1 : team.channels[channelPos].losses - 1,
  ...
  switch (args(message)) {
    case "createteam": {
        ...
        await message.guild.roles
          ...
          .catch((e) =>
            message.channel.send(
              "Error creating role Team Captain, please check if there is already a role with that name"
            )
          );
      }
      ...
      await message.guild.roles
        ...
        .catch((e) =>
          message.channel.send(
            `Error creating role ${messageArgs(message)}, please check if there is already a role with that name`
          )
        );

      await message.member.roles
        .add(getroleID(messageArgs(message)))
        .catch((e) => message.channel.send(`Error adding role ${messageArgs(message)}, please check if it exists`));

      await message.member.roles
        .add(getroleID("Team Captain"))
        .catch((e) =>
          message.channel.send(
            "Could not add role Team captain, make sure that the role exists and if not, ask an admin to create it"
          )
        );
    ...
    case "disband": {
      if (messageArgs(message) !== "" && message.member.hasPermission("ADMINISTRATOR")) {
        ...
        await (
          await message.guild.members.fetch(findGuildTeams.find((e) => e.name === messageArgs(message)).members[0])
        ).roles
          .remove(getroleID("Team Captain"))
          .catch((e) =>
            message.channel.send(
              "Could not remove role Team captain, make sure that the role exists and if not, ask an admin to create it"
            )
          );

        message.guild.roles.cache
          .find((role) => role.name === messageArgs(message))
          .delete()
          .catch((e) => message.channel.send(`Error trying to delete role ${messageArgs(message)}`));
      ...
      for (const team of findGuildTeams) {
        if (team.members.indexOf(userId) === 0) {
          message.guild.roles.cache
            .find((role) => role.name === team.name)
            .delete()
            .catch((e) => message.channel.send(`Could not delete role ${team.name}, please check if the role exists.`));
    ...
    case "giveownership": {
      ...
      await (await message.guild.members.fetch(getIDByTag(secondArg))).roles
        .add(getroleID("Team Captain"))
        .catch((e) =>
          message.channel.send(
            "Could not add role Team captain, make sure that the role exists and if not, ask an admin to create it"
      ...
      correctEmbed.setTitle(
        `:white_check_mark: Given ownership to ${(await fetchFromID(getIDByTag(secondArg))).username}`
      );
    ...
    case "leaveteam": {
      ...
      await message.member.roles
        .remove(getroleID(teamsInfo().name))
        .catch((e) =>
          message.channel.send(`Could not delete role ${teamsInfo().name}, please check if the role exists.`)
        );
    ...
    case "kickplayer": {
      ...
      correctEmbed.setTitle(
        `:white_check_mark: ${message.author.username} just kicked ${
          (await fetchFromID(getIDByTag(secondArg))).username
        } from ${leftTeam}`
      );
    ...
    case "invite": {
      ...
      correctEmbed.setTitle(
        `:white_check_mark: Invited ${(await fetchFromID(getIDByTag(secondArg))).username} to ${teamsInfo().name}!`
      );
    ...
    case "jointeam": {
      ...
      await message.member.roles
        .add(getroleID(messageArgs(message)))
        .catch(
          (e) => `Could not add role ${messageArgs(message)}, please check if the role exists and if not create it`
        );
    ...
    case "pendinginvites": {
      ...
      wrongEmbed.setDescription(
        Object.keys(invites)
          .filter((e) => invites[e].includes(userId))
          .join(", "),
        "Show what you can do in order to get more invites!"
      );
      ...
      switch (secondArg) {
          ...
          for (let j = 0; j < findGuildTeams.length; j++) {
            if (findGuildTeams[j].name === teamsInfo().name) {
              const scoreDirectory =
                findGuildTeams[j].channels[
                  findGuildTeams[j].channels.map((e) => e.channelID).indexOf(message.channel.id)
                ];
        ...
        case "channel": {
          const getScore = (id, arg) => {
            const teamScores = findGuildTeams.filter(
              (a) =>
              ...
              for (indexes; indexes < 20 * arg; indexes++) {
                ...
                for (const channels of teamScores[indexes].channels) {
                  if (channels.channelID === id) {
                    correctEmbed.addField(
                      teamScores[indexes].name,
                      `Wins: ${channels.wins} | Losses: ${channels.losses} | Winrate: ${
                        isNaN(Math.floor((channels.wins / (channels.wins + channels.losses)) * 100))
                          ? "0"
    ...
    case "reset": {
      ...
      switch (secondArg) {
        ...
        case "team": {
          ...
          const channelPos = findGuildTeams[findGuildTeams.map((e) => e.name).indexOf(thirdArg)].channels
            .map((e) => e.channelID)
            .indexOf(channel_ID);
    ...
    case "q": {
      ...
      if (teamsArray.length === 2) {
        ...
        for (const team of teamsArray) {
          ...
          const channelsInDatabaseSpecific = `teams.${findGuildTeams.indexOf(
            teamsInfoSpecific(team.members[0])
          )}.channels`;
