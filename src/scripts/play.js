...
const execute = async (message) => {
  ...
  const play = () => {
    ...
    voiceChannel.join().then((connection) => {
      connection
        .play(
          yt(`https://www.youtube.com/watch?v=${musicQueue[0].videoID}}`, {
  ...
  rp(
    ` https://www.googleapis.com/youtube/v3/search?part=snippet&maxResults=1&q=${message.content.slice(
      6
    )}&type=video&key=${process.env.ytapi}`
  ).then((res) => {
