...
const updateOngoingGames = async () => {
	...
	for (const game of ongoingGames.filter(game => currentTimeMS - game[6].time > MAX_GAME_LENGTH_MS)) {
		const channels = await client.channels.fetch(game[6].channelID).then(e => e.guild.channels.cache.array());
	...
	case 'ongoinggames': {
		...
		for (i = 0; i < 6; i++) {
			...
			if (game[6].channelID === channel_ID) {
				...
				correctEmbed.addField(':small_orange_diamond: -Team 1-', `<@${game[0].id}>, <@${game[1].id}>, <@${game[2].id}>`);
				correctEmbed.addField(':small_blue_diamond: -Team 2-', ` <@${game[3].id}>, <@${game[4].id}>, <@${game[5].id}>`);
